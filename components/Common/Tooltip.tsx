    ...
    >
      ...
      {show && (
        <div className={`absolute left-1/2 -translate-x-1/2 z-50 animate-in fade-in zoom-in-95 duration-150 ${position === 'top' ? 'bottom-full mb-3' : 'top-full mt-3'} ${width}`}>
          <div className="bg-[var(--bg-primary)]/95 backdrop-blur-md border border-[var(--status-info)] text-[var(--text-primary)] text-xs p-3 shadow-[0_0_20px_rgba(0,0,0,0.5)] relative">
            ...
            {/* Pointer Arrow */}
            <div className={`absolute left-1/2 -translate-x-1/2 w-2 h-2 bg-[var(--bg-primary)] border-r border-b border-[var(--status-info)] rotate-45 ${position === 'top' ? '-bottom-1.5' : '-top-1.5 rotate-[225deg]'}`}></div>
